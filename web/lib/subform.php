...
if ( substr(phpversion(),0,1) != 5) {
    if (!function_exists('clone')) {
        // emulate clone  - as per php_compact, slow but really the correct
behaviour..
...
/**
* A few caveats: this element *must* be either created by a call to addElement on
your main form (with the
* construction parameters, not an element object) or you must call setParentForm on
the element. If you
* don't, the rules on the subform won't work. Here's the two usages (assuming $form
is the main form
...
*
* require_once('HTML/QuickForm/SubForm.php');
* $el =& HTML_QuickForm::createElement('subFormElementName', 'Sub Form Label',
$subForm);
* $el->setParentForm($form);
* $form->addElement($el);
*
* This also uses a few hacks which access HTML_QuickForm internals which is a
no-no, but it's the only
* way I could get unfreeze and setPersistentFreeze to work as HTML_QuickForm
doesn't implement these
* functions (perhaps these should be added?). This also only works with the default
QF renderer, but
...
"showSubFormElement()">Show Sub Form</a>
* <div class="'.($_REQUEST['subFormDisplayed'] ? '' : 'hidden')." id=
"idForElementDiv">{element}</div>',
...
class HTML_QuickForm_SubForm extends HTML_QuickForm_static {
    ...
    function toHtml()
    {
        ...
        $this->_renderer->setFormTemplate(preg_replace('!</?form[^>]*>!', '',
...
if (class_exists('HTML_QuickForm')) {
    HTML_QuickForm::registerElementType('subForm', 'HTML/QuickForm/SubForm.php',
