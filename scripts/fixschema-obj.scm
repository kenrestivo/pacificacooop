...
(define-method (process-def (self <schema>) line)
  (cond ((and (equal? (car line) "create")
			  (equal? (cadr line) "table"))
		 (set-slot! self current-table 
					(unparen (caddr line))))
		((and (equal? (car line) "primary")
			  (equal? (cadr line) "key"))
		 (add-primary-key (slot-ref self current-table) line)
		 (else (add-column (slot-ref self current-table) line)))))
