...
$GLOBALS['_CRM_CONTACT_BAO_CONTACT']['_commPrefs'] =  array( 'do_not_phone', 'do_not_email', 'do_not_mail', 'do_not_trade' );
$GLOBALS['_CRM_CONTACT_BAO_CONTACT']['_importableFields'] =  null;
$GLOBALS['_CRM_CONTACT_BAO_CONTACT']['_exportableFields'] =  null;
...
ORDER BY
  civicrm_location.is_primary DESC, civicrm_email.is_primary DESC";
     ...
     * @static
     */
     function allPhoneNumbers( $id, $type = null ) {
        if ( ! $id ) {
            return null;
        }
        ...
        if ( array_key_exists( 'location', $defaults ) ) {
            ...
            foreach ($locations as $index => $location) {                
                ...
                if (array_key_exists( 'address', $location ) ) {
                    if ( ! CRM_Contact_BAO_Contact::lookupValue( $location['address'], 'state_province',
                                              CRM_Core_PseudoConstant::stateProvince(), $reverse ) &&
                         $reverse ) {
                        CRM_Contact_BAO_Contact::lookupValue( $location['address'], 'state_province', 
                                           CRM_Core_PseudoConstant::stateProvinceAbbreviation(), $reverse );
                    }
                    
                    if ( ! CRM_Contact_BAO_Contact::lookupValue( $location['address'], 'country',
                                              CRM_Core_PseudoConstant::country(), $reverse ) &&
                         $reverse ) {
                        CRM_Contact_BAO_Contact::lookupValue( $location['address'], 'country', 
     ...
     */
     function retrieve( &$params, &$defaults, &$ids ) {
        ...
        $locParams = $params + array('entity_id' => $params['contact_id'],
                                     'entity_table' => CRM_Contact_BAO_Contact::getTableName());
        ...
        $defaults['openActivity'] = array(
                                          'data'       => CRM_Contact_BAO_Contact::getOpenActivities( $activityParam, 0, 3 ),
                                          'totalCount' => CRM_Contact_BAO_Contact::getNumOpenActivity( $params['contact_id'] ),
                                          );
        return $contact;
    }
...
WHERE civicrm_contact.id IN $idString AND civicrm_address.geo_code_1 is not null AND civicrm_address.geo_code_2 is not null";
     ...
     * @access public
     */
     function getNumOpenActivity($id) {
...
";
     ...
     * @static
     */
     function &makeHierReturnProperties( $fields ) {
     ...
     * @static
     */
     function getPrimaryLocationType($contactId) 
    {
        require_once 'CRM/Core/BAO/Location.php';
        $location =& new CRM_Core_DAO_Location( ); 
        $location->entity_table = 'civicrm_contact';
     ...
     * @access public
     */
     function getAllContactTags( $contactId ) {
